# MacBook Pro keyboard settings for German layout.
#
# Only applied to MacBook Pro 14,2 hardware profile.
# Fixes: @ symbol, ^ and < keys being swapped.
{ lib, ... }:
{
  dendritic.nixos.apple-macbook-pro-14-2 = {
  ...
  dendritic.home.apple-macbook-pro-14-2 = { lib, ... }:
    lib.mkIf lib.stdenv.hostPlatform.isLinux {
      xdg.configFile."mango/config.conf".text = lib.mkForce ''
        # Environment
